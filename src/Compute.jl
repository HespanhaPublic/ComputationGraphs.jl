...
@inline compute_node!(
    ::ComputationGraph,
    node::Node
) where {Node<:AbstractNode} = compute_node!(node)
@inline compute_node!(
    graph::ComputationGraph,
    id::Int
) = compute_node!(graph.nodes[id])
...
@inline compute_with_ancestors!(
    graph::ComputationGraph,
    node::Node
) where {Node<:AbstractNode} = compute_with_ancestors!(graph, node.id)
@inline compute_with_ancestors!(
    graph::ComputationGraph,
    id::Int
) = do_ccall(graph.compute_with_ancestors[id])
...
@inline compute!(
    graph::ComputationGraph,
    nodes::NT
) where {NT<:NamedTuple} = compute!(graph, values(nodes))
...
function Base.get(
    ...
    compute!(graph, nodes)
    # FIXME Does not seem to be allocation free
    return Tuple(nodeValue(node) for node in nodes)
end
function Base.get(
    graph::ComputationGraph,
    nodes::NT
...
"""
function computeSpawn!(graph::ComputationGraph)
...
"""
function syncValid(graph::ComputationGraph)
    for (id, node) in enumerate(graph.nodes)
        if graph.validValue[id]
            notify(graph.validEvent[id])
        else
...
"""
function computeUnspawn!(graph::ComputationGraph)
...
"""
@inline function request(graph::ComputationGraph, node::Node
) where {Node<:AbstractNode}
    #@printf("   requesting computation by node %d\n", node.id)
    notify(graph.requestEvent[node.id])
end
@unroll function request(graph::ComputationGraph, nodes::Tuple
)
    @unroll for node in nodes
        request(graph, node)
    end
end
@inline request(graph::ComputationGraph, nodes::NamedTuple
) = request(graph, values(nodes))
...
Presumes a previous call to `computeSpawn!(graph)`
"""
@inline function Base.wait(graph::ComputationGraph, node::Node
) where {Node<:AbstractNode}
    #@printf("   waiting for node %d\n", node.id)
    wait(graph.validEvent[node.id])
end
@unroll function Base.wait(graph::ComputationGraph, nodes::Tuple
)
    @unroll for node in nodes
        wait(graph, node)
    end
...
@inline Base.wait(graph::ComputationGraph, nodes::NT) where {NT<:NamedTuple} =
