...
@testset "NN: check correctness" begin
    ...
    gradientSerial = copy.(values(get(graph, optimizerNodes.gradients)))
    #display(graph)
...
BenchmarkTools.Trial: 227 samples with 1 evaluation per sample.
 Range (min … max):  6.510 ms …  11.537 ms  ┊ GC (min … max): 0.00% … 0.00%
 Time  (median):     7.742 ms               ┊ GC (median):    0.00%
 Time  (mean ± σ):   7.926 ms ± 937.190 μs  ┊ GC (mean ± σ):  0.00% ± 0.00%

     ▂▃ ▆ ▃▄   ▅ ▂█▂▃  ▃▂▅         ▃                           
  ▅▅▆██▆████▅▇███████▅▅████▇▅▄▄▅▄▇▄█▆▃▅▄▅▃▃▄▄▁▅▁▃▁▁▃▁▁▁▁▄▁▁▃▃ ▄
  6.51 ms         Histogram: frequency by time        10.8 ms <
...
(Base.JLOptions()).opt_level = 2
Unspawning computation nodes: unspawn     0 tasks (0 failed)
Spawning computation nodes  : spawn      51 tasks
BenchmarkTools.Trial: 227 samples with 1 evaluation per sample.
 Range (min … max):  5.837 ms …  12.985 ms  ┊ GC (min … max): 0.00% … 0.00%
 Time  (median):     6.977 ms               ┊ GC (median):    0.00%
 Time  (mean ± σ):   7.204 ms ± 959.168 μs  ┊ GC (mean ± σ):  0.00% ± 0.00%

          ▃▁▁▁█  ▃ ▂  ▄                                        
  ▃▃▃▃▅█▆▆██████████▆▅█▆▄▆▆▆▃▆▆▄▃▃▁▄▃▃▁▁▃▁▁▃▁▁▁▃▁▃▁▁▁▄▃▃▁▃▁▁▃ ▃
  5.84 ms         Histogram: frequency by time        10.3 ms <

 Memory estimate: 0 bytes, allocs estimate: 0.
Int(maximum(graph.count)) = 228
Unspawning computation nodes: unspawn    51 tasks (0 failed)
Improvement of parallel over serial = 765.038 us (11.0 %)
=#
